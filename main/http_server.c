...
// Wueue handle used to manipulate the main queue of events
...
{
	https_server_queue_message_t msg;
...
static esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	if (esp_ota_end(ota_handle) == ESP_OK) {
		...
		if (esp_ota_set_boot_partition(update_partition) == ESP_OK) {
			...
			ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype %d at offset 0x%"PRIX32, boot_partition->subtype, boot_partition->address);
...
/**
 * DHT sensor readings JSON responds with DHT22 sensor data
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
...
static esp_err_t http_server_wifi_connect_json_handler(httpd_req_t *req)
{
	...
	if (len_ssid > 1) {
		...
		if (httpd_req_get_hdr_value_str(req, "my-connect-ssid", ssid_str, len_ssid) == ESP_OK) {
			ESP_LOGI(TAG, "http_server_wifi_connect_json_handler: Found Handler => my-connect-ssid: %s", ssid_str);
		}
	}
	...
	if (len_pass > 1) {
		...
		if (httpd_req_get_hdr_value_str(req, "my-connect-pwd", pass_str, len_pass) == ESP_OK) {
			ESP_LOGI(TAG, "http_server_wifi_connect_json_handler: Found Handler => my-connect-pwd: %s", pass_str);
		}
	}
...
static httpd_handle_t http_server_configure(void)
{
	...
	http_server_monitor_queue_handle = xQueueCreate(3, sizeof(https_server_queue_message_t));
	
	// The core that the HTTP server will run

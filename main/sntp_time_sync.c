...
static void sntp_time_sync_init_sntp(void)
{
	...
	/*ip_addr_t sntp_server_addr;

	if (ipaddr_aton("216.239.35.0", &sntp_server_addr)) {
	    sntp_setserver(0, &sntp_server_addr);
	} else {
	    ESP_LOGE("SNTP", "Invalid IP address format");
	}*/
...
static void sntp_time_sync_obtain_time(void)
{
	...
	if (time_info.tm_year < (2016 - 1900)) {
		...
		// Set the local time zone (Kyiv/Ukraine)
...
void sntp_time_sync_task_start(void)
{
	...
	xTaskCreate(&sntp_time_sync, "sntp_time_sync", SNTP_TIME_SYNC_TASK_STACK_SIZE, NULL, SNTP_TIME_SYNC_TASK_PRIORITY, NULL);
